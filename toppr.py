        ...
        for i in range(1, 3):
            ...
            if i == 1:
                ...
                plot.plot(
                    bandwidth_list,
                    n_h0,
                    color=[133 / 255, 185 / 255, 190 / 255],
                    linestyle="-",
                    linewidth=4,
                )
                plot.scatter(
                    bandwidth_list, n_h0, color=[133 / 255, 185 / 255, 190 / 255], s=100
                )
                ...
                plot.plot(
                    bandwidth_list,
                    s_h0,
                    color=[255 / 255, 110 / 255, 97 / 255],
                    linestyle="-",
                    linewidth=4,
                )
                plot.scatter(
                    bandwidth_list, s_h0, color=[255 / 255, 110 / 255, 97 / 255], s=100
                )

    try:
        if sum(s_h0 == max(s_h0)) == 1:
            if confidence_band == True:
                return bandwidth_list[s_h0.tolist().index(max(s_h0))], cn_list[
                    s_h0.tolist().index(max(s_h0))
                ]
    ...
    # use balloon estimator to calculate Top P&R
        ...
        for iloop in range(len(dist)):
            if iloop == 0:
                ...
                balloon_est = np.append(
                    balloon_est, sorted(dist[iloop, (iloop + 1) :])[k - 1]
                )
        balloon_est = sorted(balloon_est)
        bandwidth_r = balloon_est[
            int(len(balloon_est) * 0.5) - 1
        ]  # median balloon estimated bandwidth
